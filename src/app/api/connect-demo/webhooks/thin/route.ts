...
import { syncConnectedAccountSignals } from "@/lib/connect-demo/store";
import {
...
async function syncLatestSignalsFromStripe(args: {
  ...
  // Fetch the latest account state so we always persist the freshest requirements/capability status.
...
export async function POST(request: Request) {
  ...
  try {
    // Latest SDK uses parseEventNotification for thin events.
    // (Older examples may reference parseThinEvent.)
  ...
  try {
    // Required by thin-event flow: retrieve full event details from /v2/events.
    ...
    switch (event.type) {
      // 1) Account requirements updated.
      ...
      // 2) Merchant capability status updated.
      ...
      // 3) Customer capability status updated.
      ...
      // 4) Recipient capability status updated.
